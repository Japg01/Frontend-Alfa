...
// VAR
  ...
  Widget _buildRegisterButton() {
    return ElevatedButton(
      onPressed: () {
        if (acceptTerms) {
          if (nameController.text.isNotEmpty && //verifica si los campos están llenos
  ...
  Future<void> _registerUser() async {
    ...
    final url = Uri.parse('http://172.21.112.1:3900/auth/register'); //se utiliza para acceder al endpoint de la back
    final response = await http.post( //realizamos una solicitud http POST con los datos del usuario 
      url,
      body: {
